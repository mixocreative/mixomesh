...
## Live verification — 2026-06-11 (Bundle 1)

- `npm run test:browser` (app-shell smoke, real headless Chrome): **PASS** —
  boots clean with the new TextureReadback module, no console errors.
- `npm run test:export` (NEW functional smoke, `tests/browser-export-smoke.mjs`):
  **PASS** — imports `tests/fixtures/textured-quad.glb` (generated by
  `tests/fixtures/make-textured-quad.mjs`) through the real
  `AssetLoader.loadFromBlob`, runs the real `PrintManager.exportThreeMF`
  against the Mimaki profile with the save picker stubbed, and asserts:
  package contains `m:texture2d` + `m:texture2dgroup`, exactly one
  `3D/Textures/*.png`, valid PNG structure, and **non-blank inflated pixel
  data** (the all-zero signature of the C1 bug). Filename contract
  `Untitled_r1to1.3mf` asserted.
- Implements architecture-review item A2 (automated functional smoke); the
  recurring "live Chrome milestone deferred" debt is closed for the texture
  export path.
- Still TODO (A2 follow-ups): pixel-order assertion for Y-flip orientation
  (decode exported 2×2 PNG, assert red texel position), MIXOMESH re-import
  round-trip inside the smoke, and a real-slicer acceptance check when a
  Mimaki/3MF slicer is available.
