...
function _renderTexturePickCard(tex, isCurrent) {
  const thumb = tex.thumbnailDataUrl
    ? `<img src="${_escape(tex.thumbnailDataUrl)}" alt="">`
    : `<div class="tp-card-thumb-empty">${icon('Image', { width: 28, height: 28 })}</div>`;
  return `
    <button class="tp-card ${isCurrent ? 'tp-card-current' : ''}"
            data-asset-id="${tex.id}"
            title="${_escape(tex.filename)}">
      ${thumb}
...
function _renderConflictRow(c) {
  ...
  return `
    <li class="mm-row" data-name="${_escape(c.name)}">
...
function _renderListRow(sh) {
  ...
  return `
    <li class="sp-row ${selected}" data-shader-id="${sh.id}">
      ${renderShaderPreview(sh, 18)}
      <span class="sp-row-name" title="${_escape(sh.name)}">${_escape(sh.name)}</span>
      <span class="sp-row-count" title="Meshes using this shader">${sh.linkedMeshIds.length}</span>
      <button class="sp-row-act" data-sp-dup="${sh.id}" title="Duplicate this shader">${icon('Copy', { width: 12, height: 12 })}</button>
    </li>
  `;
}
...
export function renderShaderPreview(sh, size = 18) {
  const tex = sh.diffuseTextureAssetId
    ...
    : null;
  if (tex?.thumbnailDataUrl) {
    return `<span class="sp-swatch-chip sp-swatch-tex" style="width:${size}px;height:${size}px;background-image:url('${_escape(tex.thumbnailDataUrl)}')"></span>`;
  }
  return `<span class="sp-swatch-chip" style="width:${size}px;height:${size}px;background:${_escape(sh.diffuseColor)}"></span>`;
}
  ...
  return `
    <section class="sp-section" data-section="editor">
      ...
      <div class="sp-row-form">
        <label>Name</label>
        <input type="text" id="sp-edit-name" value="${_escape(sh.name)}">
      </div>
      ...
      <div class="sp-row-form">
        ...
        <div class="sp-color-line">
          <input type="color" id="sp-edit-color" value="${_escape(sh.diffuseColor)}">
          <input type="text"  id="sp-edit-color-hex" value="${_escape(sh.diffuseColor)}" spellcheck="false">
        </div>
      </div>

      <div class="sp-row-form">
        <label>Texture</label>
        <div class="sp-tex-drop" id="sp-edit-tex" data-shader-id="${sh.id}">
      ...
      <div class="sp-actions">
        ...
        <button class="sp-btn sp-btn-danger" id="sp-act-delete" ${linked ? 'disabled' : ''} title="${linked ? `Refuses while ${linked} mesh${linked === 1 ? '' : 'es'} use this shader` : 'Delete shader'}">${icon('Trash2', { width: 12, height: 12 })}<span>Delete</span></button>
      </div>
    </section>
  `;
}
...
function _renderTextureSlot(sh) {
  ...
  return `
    <span class="sp-tex-loaded" title="${_escape(asset.filename)}">
      <img src="${_escape(asset.thumbnailDataUrl)}" alt="">
...
function _renderSwatchChip(sw) {
  return `
    <button class="sp-swatch"
            data-hex="${_escape(sw.hex)}"
            title="${_escape(sw.name)} — ${_escape(sw.hex)}"
            style="background:${_escape(sw.hex)}">
    </button>
  `;
}
